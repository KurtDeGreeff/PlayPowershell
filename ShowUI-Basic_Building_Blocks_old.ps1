...
New-Window -Title "Network Config" -HorizontalContentAlignment Center -WindowStartupLocation CenterScreen -Height 610 -Width 400 -WindowStyle ThreeDBorderWindow -ShowActivated -Show {

    New-StackPanel -Name IpStack -Background LightBlue {
    ...
    New-StackPanel -Name PdcStack  -Children {
        ...
        New-Button -IsDefault -VerticalAlignment Bottom -Margin 10 -MaxWidth 150 -FontSize 15 -Content "Join to Domain" -On_Click { 
        $hash = Get-ParentControl |  Set-UIValue -passThru
        (Get-NetConfig).EnableStatic("$hash.ip","$hash.mask")
        }
