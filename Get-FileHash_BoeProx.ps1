function Get-FileHash { 
    ...
    Process {  
        ForEach ($item in $Path) { 
            $item = $item -replace "Microsoft.PowerShell.Core\\FileSystem::",""
           ...
           $object = New-Object PSObject -Property @{ 
            ...
            foreach($Type in $Algorithm) {
                [string]$hash = -join ([Security.Cryptography.HashAlgorithm]::Create( $Type ).ComputeHash( ([IO.StreamReader]$item).BaseStream ) | ForEach { "{0:x2}" -f $_ })
